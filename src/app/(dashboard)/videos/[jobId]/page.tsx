...
import { VideoAssetsEditor } from "~/app/(dashboard)/videos/_components/videoAssetsEditor";
...
export default function Page() {
  ...
  const { data: script } = api.job.getScript.useQuery(
    { jobId },
  ...
  if (pageStatus === "script_generated") {
    return (
      <UploadAssetField
        jobId={jobId}
        character1Name={script?.script?.character1?.name ?? "Character 1"}
        character2Name={script?.script?.character2?.name ?? "Character 2"}
        // TODO (you): wire these handlers
        // onContinue={(selections) => { selectAssetForJob(...) then continuePipeline(...) }}
        // onSkip={() => { continuePipeline({ jobId }) }}
      />
    );
  }
  ...
  // ─── Stage 5: Complete — show video editor ───
