...
export const userRole = ["user", "subscriber", "admin"] as const;
export type UserRoleType = (typeof userRole)[number];

export const userPlan = ["free", "pro"] as const;
export type UserPlanType = (typeof userPlan)[number];

export const userTable = createTable("user", {
  userId: text("user_id").primaryKey(),
  role: text("role").$type<UserRoleType>().default("user").notNull(),
  userPlan: text("user_plan").$type<UserPlanType>().default("free").notNull(),
  dailyCreditsGrantedAt: timestamp("daily_credits_granted_at"),
